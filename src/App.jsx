...
const App = () => {
  ...
  const generateProblem = useCallback(() => {
    ...
    if (op === 'add') {
      ...
      probString = `${a} * ${b}`; // Using correct multiplication symbol like original
      ans = a * b;
    } else if (op === 'div') {
      const a = Math.floor(Math.random() * (settings.mulMax1 - settings.mulMin1 + 1)) + settings.mulMin1;
      const b = Math.floor(Math.random() * (settings.mulMax2 - settings.mulMin2 + 1)) + settings.mulMin2;
  ...
  const downloadCSV = () => {
    ...
    // If running in the Desktop App
    if (window.electronAPI) {
      window.electronAPI.saveCSV(fileName, csvData);
    } 
    // Fallback if running in a normal web browser
    else {
    ...
    <>
      {gameState === 'menu' && (
        <div id="menu-container">
          ...
          {/* Adding the exact horizontal line from the original layout */}
      ...
      {gameState === 'result' && (
        <div id="result-screen">
          ...
          <div className="csv-container">
            <label>Raw Data (Paste directly into Sheets):</label>
            <textarea 
              id="csv-output" 
              readOnly 
              ref={csvOutputRef}
